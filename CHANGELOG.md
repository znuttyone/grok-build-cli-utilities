...
- **`usage cost --by pr`**: when a session created exactly one PR, all of that session's list$ goes to `repo#N` (issue from `Fixes #N` when the create body has it). Two or more PRs stay one unsplit row (`ProfitGuard #69,71` or mixed `ProfitGuard #7,8,14 · grok-build-cli-utilities #15`). A one-line footnote marks unsplit multi-PR rows. No GitHub owner. No session UUID in the key unless two rows share a display Key (`Blessed-Bits#22 · 01a05e3c…`). Unlabeled `--by session` and `--by app` Keys pretty-print `*-issue-N` clones and Grok worktrees without folding spend into the parent app. Sessions with no created PR are omitted from `--by pr` unless `--include-unlabeled`. Chat text and `get_pull_request` are ignored.
