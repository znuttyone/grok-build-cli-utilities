...
### Added
- **`usage cost` / `usage report --all`**: print every bucket. Default `--top` is 10 by list$ (cost was 8). Truncated tables say `top 10 of 18`. `--all` overrides `--top`. TOTALS is the whole window. Hidden rows stay uncombined.
- **`usage cost --by session`** and **`--by pr`**: `--by app` is the folder (`basename` of cwd). One Grok Build chat in that folder is one cost bucket even when it shipped many PRs. `--by session` is one row per chat. `--by pr` is one row per GitHub PR when that chat created exactly one PR (`repo#N`, issue from `Fixes` or `Closes`). Several PRs from one parent chat stay one unsplit row (`widgets #12,15` or mixed `widgets #7,8,14 · notes #15`). A one-line footnote marks unsplit multi-PR rows. No GitHub owner. No session UUID in the key unless two rows share a display Key (`notes#22 · 01a05e3c…`). Unlabeled `--by session` and `--by app` Keys pretty-print `*-issue-N` clones (`notes-issue-9` → `notes#9`) and Grok worktrees without folding spend into the parent app. Sessions with no created PR are omitted from `--by pr` unless `--include-unlabeled`. Chat text and `get_pull_request` are ignored. PR-level Keys need github `create_pull_request` or `gh pr create` in `updates.jsonl`, with session cwd in that repo.
