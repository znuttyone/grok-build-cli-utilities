...
- **`usage cost --by pr`**: when a session created exactly one PR, all of that session's list$ goes to `owner/repo#N` (issue from `Fixes #N` when the create body has it). Two or more PRs stay one unsplit row (`znuttyone/ProfitGuard #69,#71,…`). No session UUID in the key. Sessions with no created PR are omitted unless `--include-unlabeled`. Chat text and `get_pull_request` are ignored.
